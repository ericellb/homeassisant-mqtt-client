...
const deleteEvent = async (
  ...
  const commands = (dataCopy.find(tc => tc.topic === selected.topic) as TopicCommand).commands.filter(
    c =>
      !(
        ((selected.event.type === CommandTypes.APPLICATION &&
          c.type === CommandTypes.APPLICATION &&
          c.path === selected.event?.path) ||
          (selected.event.type === CommandTypes.NIRCMD &&
            c.type === CommandTypes.NIRCMD &&
            c.command === selected.event.command)) &&
